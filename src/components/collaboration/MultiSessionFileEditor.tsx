...
export default function MultiSessionFileEditor({ 
  ...
  const activeFile = useQuery(
    api.sessionFiles.getSessionFile,
    activeTabId && user?.id ? { 
      fileId: activeTabId, 
      sessionId,
      userId: user.id 
  ...
  }, [activeTabId, unsavedChanges, updateFile, user?.id, sessionId]);
    ...
    if (activeFile && activeTabId) {
      setTabs(prevTabs => {
        ...
        if (existingTabIndex === -1) {
          ...
          return prevTabs; // No changes needed
        }
      });
    }
    ...
    <div className="flex flex-col h-full">
      ...
      {showOutput && (
        <div className="h-1/2 border-t border-white/[0.05] flex flex-col">
          ...
          <div className="flex-1 p-4 bg-[#12121a] overflow-auto vscode-scrollbar">
            {error ? (
